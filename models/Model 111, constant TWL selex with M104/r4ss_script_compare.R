...
library(r4ss)
model.dir <- "Model 111, constant TWL selex with M104"
...
dir3 <- "E:/ejdick/projects/~Chilipepper 2025/model/Model 110, 2D-AR selectivity with M104"
...
mod.sum <- SSsummarize(list(mod1, mod2, mod3)) #, mod3, mod4, mod5))

# legend labels and table column names
legend.labels <- c("constant TWL selex", "M104 base", "2D TWL selex")
...
##### HAD TO ADD PARM DEVS TO COMPARISON TABLE -- DEVS ADD >1000 NLL POINTS
...
likelihood <- mod.sum$likelihoods[mod.sum$likelihoods$Label %in%
                                  c("TOTAL","Survey","Length_comp","Age_comp","Recruitment","Parm_priors","Parm_devs"), c(mod.sum$n+1,1:mod.sum$n)]
