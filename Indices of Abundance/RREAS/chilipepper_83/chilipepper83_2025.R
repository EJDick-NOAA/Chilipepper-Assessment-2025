...
ggsave(paste0(fig.dir,spname,"_2_sampling_effort_st.png"), width = 15, height = 6)
...
ggplot(logind, aes(x=YEAR, y=N100_INDEX)) +
  ...
  scale_x_continuous(breaks = seq(1983,2025,by=2)) + theme_bw() +
  labs(title=paste(spname,"raw cpue"))
ggsave(paste0(fig.dir,spname,"_3_raw_cpue.png"), width = 8, height = 4)

# sdmtmb model ####

(totals=aggregate(TOTAL_NO~YEAR, data=focalcoast, FUN=sum))
#excludeyears=totals$YEAR[totals$TOTAL_NO==0]
excludeyears=c(1983,2020)
...
ggplot(ind_comp, aes(x=YEAR, y=log_est,color=model, fill=model, group=model)) +
  ...
  scale_x_continuous(breaks = seq(1983,2025,by=2))
ggsave(paste0(fig.dir,spname,"_comparison_error_dist.png"), width = 8, height = 6)
